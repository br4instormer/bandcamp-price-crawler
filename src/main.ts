...
import { fetchPage, parseCost, random } from "./utils";
...
const DELAY_MIN = Number(config.get("DELAY_MIN"));
const DELAY_MAX = Number(config.get("DELAY_MAX"));

const worker = async (payload: QPayload): Promise<void> => {
  const { client, url, count, timeout } = payload;
...
((list: string, client: AxiosInstance, q: Queue, count: Count): void => {
  list
    .split("\n")
    .filter(Boolean)
    .forEach((url) => q.push({ client, url, count, timeout: random(DELAY_MIN, DELAY_MAX) }));
